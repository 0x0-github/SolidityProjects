    ...
    IDividendPayingTokenOptional
{
    ...
    // the amount of received ether is small.
    ...
    uint256 internal withdrawGas = 5000;

    // About dividendCorrection:
    // If the token balance of a `_user` is never changed, 
    // the dividend of `_user` can be computed with:
    //   `dividendOf(_user) = dividendPerShare * balanceOf(_user)`.
    // When `balanceOf(_user)` is changed (via minting/burning/transferring),
    //   `dividendOf(_user)` should not be changed,
    //   but the computed value of `dividendPerShare * balanceOf(_user)`
    // is changed.
    // To keep the `dividendOf(_user)` unchanged, we add a correction term:
    //   `dividendOf(_user) = dividendPerShare * balanceOf(_user)
    //    + dividendCorrectionOf(_user)`,
    // where `dividendCorrectionOf(_user)` is updated
    // whenever `balanceOf(_user)` is changed:
    //   `dividendCorrectionOf(_user) = dividendPerShare
    //   * (old balanceOf(_user)) - (new balanceOf(_user))`.
    // So now `dividendOf(_user)` returns the same value before and 
    // after `balanceOf(_user)` is changed.
    ...
    /// It emits the `DividendsDistributed` event if the amount
    /// of received ether is greater than 0.
    /// About undistributed ether:
    ///   In each distribution, there is a small amount of ether 
    ///   not distributed,
    ...
    /// @dev It emits a `DividendWithdrawn` event 
    /// if the amount of withdrawn ether is greater than 0.
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            (bool success,) = user.call{value: _withdrawableDividend, gas: withdrawGas}("");
    ...
    /// @notice View the amount of dividend in wei that an address
    /// has earned in total.
    /// @dev accumulativeDividendOf(_owner) = withdrawableDividendOf(_owner)
    /// + withdrawnDividendOf(_owner)
