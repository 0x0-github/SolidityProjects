...
contract ScholarDogeTeamTimelock {
    ...
    uint256 private _baseTokenAmount;
    ...
    function nextWidthdraw() public view returns (uint256) {
        return _nextWidthdraw;
    }
    
    function baseTokenAmount() public view returns (uint256) {
    ...
    function release() public virtual {
        ...
        if (_baseTokenAmount == 0)
            _baseTokenAmount = _token.balanceOf(address(this));
    ...
    function releaseAmount() public view returns (uint256) {
        uint256 amount = _baseTokenAmount.mul(RELEASE_PERCENTAGE).div(100);
...
contract ScholarDoge is Context, Ownable, ERC20("ScholarDoge", "$SDOGE") {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint256 private constant T_TOTAL = 1000000000 * 10**6 * 10**9;
    uint256 private rTotal = (MAX - (MAX % T_TOTAL));
    uint256 private tFeeTotal;

    uint8 public taxFee;
    uint8 private previousTaxFee = taxFee;
    ...
    // Used for conversion (50/50)
    address public conversionToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    uint256 public maxTxAmount = T_TOTAL;
    uint256 public constant MIN_TO_SWAP
        = 500000 * 10**6 * 10**9;
    ...
    event ConversionTokenUpdated(address token);
    ...
    event TaxFeeUpdated(uint8 value);

    event LiquidityFeeUpdated(uint8 value);

    event TreasuryFeeUpdated(uint8 value);
    ...
    constructor() public {
        ...
        uint256 teamAlloc = rTotal.div(5);
	uint256 marketingAlloc = rTotal.div(5);
	uint256 airdropFoundationAlloc = rTotal.div(5);
	uint256 devFoundationAlloc = rTotal.div(1);
	uint256 burnFoundationAlloc = rTotal.div(4);
        uint256 alloc = rTotal.sub(teamAlloc).sub(marketingAlloc)
        ...
        pair = IPancakeV2Factory(_router.factory())
    ...
    function widthdrawLockedBnb(uint256 amount) public onlyOwner {
        ...
        emit WidthdrawnLockedBnb(amount);
    }
    
    function buyBackAndBurnWithLockedBnb(uint256 amount) public onlyOwner {
        ...
        // make the swap and burn the received tokens
        ...
        emit BoughtBackAndBurnedWithLockedBnb(amount);
    }
        ...
        contractUpdateWithDelay(6, 30 days)
    {
        ...
        _setRouter(_router);
        addLiquidity(tokenReceived, bnbReceived);
    ...
    function setConversionToken(address _token) external onlyOwner {
        conversionToken = _token;
        
        emit ConversionTokenUpdated(_token);
    }
    ...
    function _setRouter(address _router) private {
        ...
        pair = IPancakeV2Factory(router.factory())
        ...
        ) 
    {
        ...
        (
            ...
            uint256 rFee,
            uint256 rTreasury
        ) = _getRValues(tAmount, tFee, tLiquidity, tTreasury, _getRate());

        return (
            rAmount,
            tLiquidity,
            TuppledValue(rTransferAmount, tTransferAmount),
            TuppledValue(rFee, tFee),
            TuppledValue(rTreasury, tTreasury));
    }
        ...
        returns (uint256, uint256, uint256, uint256)
    {
        ...
        uint256 tTransferAmount = 
            tAmount.sub(tFee).sub(tLiquidity).sub(tTreasury);
        ...
        returns (uint256, uint256, uint256, uint256)
    {
        ...
        uint256 rTransferAmount =
            rAmount.sub(rFee).sub(rLiquidity).sub(rTreasury);

        return (rAmount, rTransferAmount, rFee, rTreasury);
    }
        ...
        override
    {
        ...
        require(amount > 0,
            "BEP20 : transfer amount must be greater than zero");
        require(amount <= maxTxAmount,
            "BEP20 : transfer amount exceeds max");

        // Ensures not taking fees on "this"
        bool takeFee = from != address(this) && to != address(this);

        if (swapAndLiquifyEnabled)
            processLiquidityTransfer();
        
        if (treasuryConversionEnabled && !inSwapAndLiquify)
            processTreasuryConversion();
    ...
    // by giving both BNB and ScholarDogeTokens
    function processTreasuryConversion() private lockTheSwap {
        ...
        if (amount >= MIN_TO_SWAP) {
            ...
            if (conversionToken == address(0x0)) {
                swapTokensForBnb(half, treasury);
            } else {
                swapTokensForTokens(half, treasury);
            }
        }
    }
    
    // Swaps half of the balance to BNB or conversion token
    ...
    function processTeamConversion(uint256 received) private lockTheSwap {
        ...
        if (conversionToken == address(0x0)) {
            swapTokensForBnb(half, teamTimelock.beneficiary());
        } else {
            swapTokensForTokens(half, teamTimelock.beneficiary());
        }
    }
    
    function processLiquidityTransfer() private {
        ...
        private lockTheSwap
    {
        ...
        // that the swap creates, and not make the liquidity event
        // include any BNB that has been manually sent to the contract
        ...
        private
    {
        ...
        swapTokensForBnb(tokenAmount, address(this));
        
        uint256 toTransfer = address(this).balance.sub(previousBalance);
        
        // generate the dex pair path of token -> wbnb
        address[] memory path = new address[](2);

        path[0] = router.WETH();
        path[1] = conversionToken;
        ...
        private
    {
        ...
        (
            uint256 rAmount,
            uint256 tLiquidity,
            TuppledValue memory transferAmount,
            TuppledValue memory fee,
            TuppledValue memory _treasury
        ) = _getValues(amount);

        rOwned[sender] = rOwned[sender].sub(rAmount);
        rOwned[recipient] = rOwned[recipient].add(transferAmount.rValue);
        rOwned[treasury] = rOwned[treasury].add(_treasury.rValue);

        _takeLiquidity(tLiquidity);
        _reflectFee(fee.rValue, fee.tValue);

        emit Transfer(sender, recipient, transferAmount.tValue);
